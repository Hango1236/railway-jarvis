...
class OpenRouterAI:
    ...
    def generate(self, user_text):
        ...
        # ===== Команды ПК =====
        if any(word in text_lower for word in ["статус пк", "пк онлайн", "комп в сети"]):
            if pc_bridge:
                if pc_bridge.check_status():
                    status = pc_bridge.get_pc_status()
                    return (
                        "✅ Компьютер в сети!\n"
                        f"⏰ Время: {status.get('time', 'unknown')}\n"
                        f"📅 Дата: {status.get('date', 'unknown')}"
                    )
                else:
...
# ================= TELEGRAM ФУНКЦИИ (ИСПРАВЛЕНО UTF-8) =================
...
def send_photo(chat_id, photo_io, caption):
    ...
    files = {
        'photo': ('screenshot.png', photo_io, 'image/png')
    }
